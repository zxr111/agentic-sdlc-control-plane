...
func BuiltinTools() []BuiltinTool {
	return []BuiltinTool{
		...
		{Key: "gitlab.comment", DisplayName: "GitLab 评论", Description: "通过事务发件箱更新 GitLab 评论", RiskLevel: "L2", AdapterType: "outbox", DefaultDecision: "ALLOW", InputSchema: `{"type":"object"}`, OutputSchema: `{"type":"object"}`},
		{Key: "staging.deploy", DisplayName: "测试环境部署", Description: "触发受工程师门禁保护的测试环境部署", RiskLevel: "L3", AdapterType: "outbox", DefaultDecision: "ALLOW", RequiresGate: true, InputSchema: `{"type":"object","required":["commit_sha"]}`, OutputSchema: `{"type":"object"}`},
