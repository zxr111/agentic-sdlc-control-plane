...
- GitLab and callback secrets are separate and compared in constant time. Bodies are limited to 2 MiB and only explicitly enabled event names are accepted.
...
- Factory never executes source changes. Visible engineer-owned Codex tasks use the engineer's checkout and identity.
- Merge is allowed only after a Code Review Gate for the exact current MR SHA and required CI. Release and production-risk actions require dedicated Engineer Gates.
- `production_enabled` defaults to false. The test deployment has no production database, cluster, registry, or application credential.
- Email relay accepts only an exact first-line command, is idempotent by message-ID hash, uses the engineer's GitLab identity, and cannot decide Release, Incident, or Production Migration Gates.
